<template>
  <PartialSection @updatePage="updatePage" :title="title" :type="type" :getter="getter"></PartialSection>
  <main ref="contentDiv" class="py-10 pt-28 px-12 flex flex-col justify-between items-center lg:pt-0">
    <div class="content flex flex-wrap justify-center gap-7">
      <div class="relative sm:pr-24 m-2 w-full rounded-lg flex flex-col-reverse justify-end gap-4 sm:flex-row">
        <select class="h-10 rounded-sm p-2 z-0 border border-black w-full sm:max-w-max" @change="sort">
          <option selected value="none">Sort By</option>
          <option scope="row" v-for="state in [
  ...
  ]" :key="state.key" :value="state.key">
            {{ state.name }}
          </option>
        </select>
      </div>
      <DisplayCard v-for="card in result.data" :key="card.id" :data="card"></DisplayCard>
    </div>
    <div class="navigation">
      <Pagination :result="result" :requestParams="requestParams" :getter="getter" :currentFilters="currentFilters"
        @updateResult="updateResult" v-if="result.data && result.last_page > 1"></Pagination>
    </div>
  </main>
</template>
<script setup>
