  ...
  <section class="form bg-gray-200 flex justify-center items-center flex-col py-16 px-2 sm:px-16">
    <form method="POST" @submit.prevent="uploadFiles"
      ...
      <div class="mb-6">
        ...
        <select v-model="form.ville_id" id="ville" class="border border-gray-600 rounded p-2 w-full">
      ...
      <div class="mb-6" v-show="allowed">
        <label for="etat" class="inline-block text-lg mb-2 required">Etat de voiture
        </label>
        <select @change="typeSelected" v-model="form.etat" id="etat" class="border border-gray-600 rounded p-2 w-full"
          name="type_annonce">
      ...
      <div class="mb-6">
        ...
        <div class="afficherImages border border-gray-200 rounded p-2 w-full">
          <label>
            <p class="text-center bg-red-500 w-max p-3 text-white my-4 mx-auto cursor-pointer hover:bg-red-700">
              Ajouter des images+
            </p>
          ...
          <div class="inserted flex justify-between mx-3" v-for="image in files" :key="image.id">
            <p>{{ image.file.name }}</p>
            <button class="outline-none border-none" @click="removeFile(image.id)">
...
const allowed = computed(() => {
  return store.getters.getUser.role == 'root' || store.getters.getUser.role == 'admin'
})
const form = ref({
  titre: null,
  description: null,
