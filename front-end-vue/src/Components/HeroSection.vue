<template>
    <section id="hero" class=" w-full relative" style="min-height: calc(100vh - 5rem);">
        <div class="image absolute top-0 left-0 w-full h-full object-cover bg-center bg-no-repeat bg-fixed "
        ...
        <div
            ...
            <div class="text-wrapper flex flex-col justify-end gap-2 ">
                <h1 class="text-white text-3xl font-mono font-bold text-center lg:text-left md:text-5xl lg:text-6xl">
                    Besoin d'une
                    voiture ?</h1><br>
                <h3 class=" text-white text-xl font-mono text-center lg:text-left md:text-3xl lg:text-4xl">Vous êtes au bon
                    endroit !</h3>
            </div>
...
const startSlider = () => {
    Interval = setInterval(() => {
        ...
        if (currCounter.value === 0) {
            document.querySelector(".image").classList.add("bg-center");
            document.querySelector(".image").classList.remove("bg-right");

        } else {
            document.querySelector(".image").classList.add("bg-right");
            document.querySelector(".image").classList.remove("bg-center");
        }
    }, 5000);
};
onMounted(() => {
