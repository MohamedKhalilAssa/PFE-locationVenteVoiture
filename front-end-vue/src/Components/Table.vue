    ...
    <div class="relative overflow-auto shadow-lg sm:rounded-lg sm:!max-h-full" style="max-height: 75vh">
        <table class="w-full text-sm text-center text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-100">
                <tr>
                    <!-- columns = [{name: ,key: },{}] -->
                    <th scope="col" class="px-6 py-3" v-for="column in columns" :key="column.key">{{ column.name }}</th>
                ...
                <tr class="bg-white border-b" v-for="row in result.data" :key="row.id">
                    ...
                    <td class=" flex px-6 py-4 space-x-4 justify-center">
                        <div v-for="action in actions" :key="action.label">
                            <router-link v-if="action.label !== 'Supprimer'"
                                :to="{ name: action.redirectToAction, params: { id: row.id } }"
                                class="font-medium text-black hover:underline" :class="action.addClass">{{ action.label
                                }}</router-link>
                            <form v-else method="post" @submit.prevent="DeleteFromDB(deleteFrom, row.id, loader)"
...
// extracting loader
const loaderFromGetter =
    Object.entries(getterElements)
        .filter(([key, value]) => key.includes('load'));
let loader = loaderFromGetter[0][1];
// extracting result
const resultFromGetter =
    Object.entries(getterElements)
        .filter(([key, value]) => key.includes('esult'));
